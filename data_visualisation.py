...
def visual(data, title, fontsize, output_file):
    ...
    categories = ["Economic", "Environmental", "Human", "Societal", "Political", "Security"]
    ...
    for i, subgroup in enumerate(data["Sub-group"]):
        ...
        for j, v in enumerate(values):
            ax.text(x[j] + i * width, v + 0.5, f'{v:.1f}', ha='center', fontsize=fontsize-3)
...
def visual4(data, year, title, fontsize, output_file, ylim=None, padding=None):
    df = pd.DataFrame(data)

    num_countries = len(df["Country"])
    df[year] = df[year].fillna(0)
    half = num_countries // 2

    fig, (ax1, ax2) = plt.subplots(2, 1, figsize=(11, 6), sharey=True)
    pil_img = Image.open('asean_map.png')
    width, height = pil_img.size
    pil_img_resized = pil_img.resize((int(width // 1.6), int(height // 1.6)))
    img_resized = np.array(pil_img_resized)
    ...
    num_years = len(year)
    index1 = np.arange(half)
    index2 = np.arange(num_countries - half)

    center_offset = bar_width * (num_years - 1) / 2

    # Custom color gradient
    base_color = np.array([0.0745, 0.220, 0.396])
    colors = [base_color + (i * 0.1) for i in range(num_years)]

    for i, yr in enumerate(year):
        # ax1 (Top half)
        bars1 = ax1.bar(index1 - center_offset + i * bar_width, df[yr][:half],
                        bar_width, label=str(yr), color=colors[i])
        for bar in bars1:
            h = bar.get_height()
        ...
        # ax2 (Bottom half)
        bars2 = ax2.bar(index2 - center_offset + i * bar_width, df[yr][half:],
                        bar_width, label=str(yr), color=colors[i])
        for bar in bars2:
            h = bar.get_height()
    ...
    if ylim:
        ...
        ax2.set_ylim(*ylim[:2])
        if len(ylim) == 3:
            ticks = np.arange(ylim[0], ylim[1] + ylim[2], ylim[2])
    ...
    handles = [ax1.bar(0, 0, bar_width, color=colors[i])[0] for i in range(len(year))]
    labels = [str(yr) for yr in year]

    fig.legend(handles=handles, labels=labels, loc="upper center", bbox_to_anchor=(0.5, 0.45),
               ncol=len(year), fontsize=fontsize - 1)
...
def environment_dimension():
    ...
    visual4(data7, years, title7, fontsize = 9, output_file = 'visuals/env_non_renewable_crime.png', ylim=(1, 10, 1), padding = 0.01)
...
def political_dimension():   
    title1 = 'Political Indicator: Arrests from online political content'
    years = [2020, 2021, 2022, 2023, 2024]
    data1 = {
            "Country": ["Cambodia", "Indonesia", "Laos", "Malaysia", "Philippines", 'Singapore',"Thailand","Vietnam"],
            2020 : [-1.078, 0.779, -1.979, 0.113, -0.631, -0.855, -1.408, -2.017],
            2021 : [-1.797, 0.779, -1.979, 0.113, -0.631, -0.855, -1.408, -2.017],
            2022 : [-1.797, 1.021, -1.979, 0.261, -0.631, -0.855, -1.408, -2.017], 
            2023 : [-1.797, 1.021, -1.979, 0.094, -0.631, -0.855, -1.408, -2.017],
            2024 : [-1.492, 0.361, -1.979, 0.094, -0.631, -0.855, -1.408, -1.736]}
        
    visual2(data1, years, title1, fontsize = 9, output_file = 'visuals/pol_arrest_online.png', ylim=(-2.5, 1.5, 0.5), padding = 0.005)    
       
    title2 = 'Political Indicator: Freedom from political killings and torture'
    years = [2019, 2020, 2021, 2022,2023]
    data2 = {
            "Country": ["Brunei","Cambodia", "Indonesia", "Laos", "Malaysia", "Myanmar","Philippines", 'Singapore',"Thailand","Vietnam"],
            2019 : [0, 0.400, 0.762, 0.137, 0.804, 0.489, 0.297, 0.936, 0.258, 0.577],
            2020 : [0, 0.412, 0.711, 0.161, 0.829, 0.495, 0.297, 0.936, 0.258, 0.627], 
            2021 : [0, 0.326, 0.758, 0.139, 0.849, 0.095, 0.263, 0.945, 0.258, 0.526],
            2022 : [0, 0.314, 0.792, 0.172, 0.849, 0.095, 0.281, 0.945, 0.258, 0.597],
            2023 : [0, 0.211, 0.769, 0.172, 0.883, 0.095, 0.304, 0.972, 0.352, 0.592]}
        
    visual2(data2, years, title2, fontsize = 9, output_file = 'visuals/pol_freedom_killing.png', ylim=(0, 1.4, 0.1), padding = 0.005)
    
    title3 = "Political Indicator: Women's political empowerment"
    years = [2019, 2020, 2021, 2022,2023]
    data3 = {
            "Country": ["Brunei","Cambodia", "Indonesia", "Laos", "Malaysia", "Myanmar","Philippines", 'Singapore',"Thailand","Vietnam"],
            2019 : [0, 0.585, 0.737, 0.494, 0.764, 0.621, 0.797, 0.803, 0.680, 0.746],
            2020 : [0, 0.589, 0.712, 0.496, 0.802, 0.651, 0.798, 0.815, 0.639, 0.699], 
            2021 : [0, 0.577, 0.704, 0.492, 0.803, 0.358, 0.792, 0.800, 0.649, 0.709],
            2022 : [0, 0.555, 0.723, 0.509, 0.788, 0.345, 0.806, 0.799, 0.651, 0.720],
            2023 : [0, 0.517, 0.693, 0.496, 0.789, 0.341, 0.806, 0.783, 0.687, 0.748]}
        
    visual2(data3, years, title3, fontsize = 9, output_file = 'visuals/pol_women.png', ylim=(0, 1.2, 0.2), padding = 0.005)
    
    title4 = 'Political Indicator: Perception of corruption'
    years = [2019, 2020, 2021, 2022, 2023]
    data4 = {
            "Country": ["Brunei","Cambodia", "Indonesia", "Laos", "Malaysia", "Myanmar","Philippines", 'Singapore',"Thailand","Vietnam"],
            2019 : [60, 20, 40, 29, 53, 29, 34, 85, 36, 37],
            2020 : [60, 21, 37, 29, 51, 28, 34, 85, 36, 36], 
            2021 : [np.nan, 23, 38, 30, 48, 28, 33, 85, 35, 39],
            2022 : [np.nan, 24, 34, 31, 47, 23, 33, 83, 36, 42],
            2023 : [np.nan, 22, 34, 28, 50, 20, 34, 83, 35, 41]}
        
    visual2(data4, years, title4, fontsize = 9, output_file = 'visuals/pol_corruption.png', ylim=(0, 110, 10), padding = 0.005)
    
    title5 = 'Political Indicator: Restrictions on political rights'
    years = [2021, 2022, 2023, 2024, 2025]
    data5 = {
            "Country": ["Brunei","Cambodia", "Indonesia", "Laos", "Malaysia", "Myanmar","Philippines", 'Singapore',"Thailand","Vietnam"],
            2021 : [6, 7, 2, 7, 4, 5, 3, 4, 7, 7],
            2022 : [6, 7, 2, 7, 4, 7, 3, 4, 7, 7],
            2023 : [6, 7, 2, 7, 4, 7, 3, 4, 6, 7], 
            2024 : [6, 7, 3, 7, 4, 7, 3, 4, 5, 7],
            2025 : [6, 7, 3, 7, 4, 7, 3, 4, 6, 7]}
        
    visual2(data5, years, title5, fontsize = 9, output_file = 'visuals/pol_res_rights.png', ylim=(0, 10, 1), padding = 0.005)
    
def security_dimension():
    title1 = 'Security Indicator: Attitudes on violence against women (%)'
    years = [2019, 2023]
    data1 = {
            "Country": ["Brunei","Cambodia", "Indonesia", "Laos", "Malaysia", "Myanmar","Philippines", 'Singapore',"Thailand","Vietnam"],
            2019 : [np.nan, 50.4, 34.5, 58.2, 41.5, 51.2, 12.9, 40.5, 8.6, 28.2],
            2023 : [np.nan, 46, 20.2, 30, 55.3, 14.4, 53.6, 13.8, 8, 56.9]}
        
    visual4(data1, years, title1, fontsize = 9, output_file = 'visuals/sec_att.png', ylim=(0, 100, 10), padding = 0.005)
    
    title2 = 'Security Indicator: Military spending (% of GDP)'
    years = [2020, 2021, 2022, 2023, 2024]
    data2 = {
            "Country": ["Brunei","Cambodia", "Indonesia", "Laos", "Malaysia", "Myanmar","Philippines", 'Singapore',"Thailand","Vietnam"],
            2020 : [3.63, 1.78, 0.89, np.nan, 1.00, 3.12, 1.30, 2.80, 1.46, np.nan],
            2021 : [3.24, 1.70, 0.74, np.nan, 0.98, 4.07, 1.41, 2.54, 1.42, np.nan],
            2022 : [2.61, 1.54, 0.77, np.nan, 0.90, 4.17, 1.32, 2.41, 1.24, np.nan], 
            2023 : [2.98, 1.54, 0.82, np.nan, 0.98, 3.75, 1.18, 2.83, 1.13, np.nan],
            2024 : [3.58, 1.52, 0.78, np.nan, 0.99, 6.79, 1.32, 2.84, 1.08, np.nan]}
        
    visual2(data2, years, title2, fontsize = 9, output_file = 'visuals/sec_military.png', ylim=(0, 10, 1), padding = 0.005)
    
    title3 = 'Security Indicator: Presence of criminal networks'
    years = [2021, 2023]
    data3 = {
            "Country": ["Brunei","Cambodia", "Indonesia", "Laos", "Malaysia", "Myanmar","Philippines", 'Singapore',"Thailand","Vietnam"],
            2021 : [2.5, 4.0, 7.5, 6.0, 5.5, 7.5, 6.0, 3.5, 5.0, 7.0],
            2023 : [2.0, 7.0, 7.5, 6.5, 5.5, 8.0, 6.5, 4.0, 6.0, 7.0]}
        
    visual4(data3, years, title3, fontsize = 9, output_file = 'visuals/sec_crim_nets.png', ylim=(0, 10, 1), padding = 0.005)
    
    title4 = 'Security Indicator: Rule of law'
    years = [2019, 2020, 2021, 2022, 2023]
    data4 = {
            "Country": ["Brunei","Cambodia", "Indonesia", "Laos", "Malaysia", "Myanmar","Philippines", 'Singapore',"Thailand","Vietnam"],
            2019 : [0.588, -0.955, -0.348, -0.969, 0.492, -1.079, -0.574, 1.838, 0.089, -0.050],
            2020 : [0.898, -0.964, -0.366, -0.883, 0.573, -1.196, -0.652, 1.835, 0.112, -0.190], 
            2021 : [0.861, -0.919, -0.253, -0.664, 0.532, -1.468, -0.667, 1.819, 0.094, -0.172],
            2022 : [0.934, -0.866, -0.188, -0.804, 0.561, -1.528, -0.518, 1.778, 0.066, -0.158], 
            2023 : [0.887, -0.819, -0.154, -0.833, 0.572, -1.617, -0.418, 1.753, 0.245, -0.085]}
        
    visual2(data4, years, title4, fontsize = 7, output_file = 'visuals/sec_rule_raw.png', ylim=(-2, 3, 0.5), padding = 0.005)
    
    title5 = 'Security Indicator: Government cybersecurity capacity'
    years = [2019, 2020, 2021, 2022, 2023]
    data5 = {
            "Country": ["Brunei","Cambodia", "Indonesia", "Laos", "Malaysia", "Myanmar","Philippines", 'Singapore',"Thailand","Vietnam"],
            2019 : [-0.001, -1.970, 0.068, -1.536, 2.733, -0.740, -0.029, 2.705, -0.001, 0.500],
            2020 : [-0.001, -1.970, 0.068, -1.378, 2.733, -0.740, -0.029, 2.705, -0.001, 0.500], 
            2021 : [-0.001, -1.280, 0.340, -1.273, 2.147, -0.646, -0.029, 2.705, -0.001, 0.725],
            2022 : [-0.001, -1.171, 1.667, -1.334, 1.806, -1.007, -0.029, 2.705, -0.001, 0.725],
            2023 : [-0.001, -1.171, 1.991, -0.629, 1.806, -1.247, -0.029, 2.705, -0.001, 0.725]}
        
    visual2(data5, years, title5, fontsize = 9, output_file = 'visuals/sec_gov_cyber.png', ylim=(-2, 4.2, 0.5), padding = 0.005)
...
data1 = {
    ...
    "Political" : [52.4, 57.3, 38.4, 42.9, 44.4],
    "Security" : [21.5, 16.3, 10.3, 21.4, 21.3]}
